...
var chart = timeseries_chart(scheme)
    .x(get_time).xLabel("Travel Time")
    .y(get_magnitude).yLabel("Vehicle")
    .brushmove(on_brush);

var PtLayer = L.pointsLayer({}, {
    radius: get_radius,
...
function catchJson(inputJson) {
    return inputJson;
}

function buttonClick() {
    console.log('u got it!');
    $.getJSON("_up_data", {}).done(function(collection) {
        PtLayer._data = collection;
        PtLayer.onRemove(map);
